...
class FileUploadsTest extends TestCase
{
    ...
    {
        ws_config()->set('livewire.temporary_file_upload.disk', 's3');
    ...
    public function the_global_upload_validation_rules_can_be_configured_and_the_error_messages_show_as_normal_validation_errors_for_the_property()
    {
        ...
        ws_config()->set('livewire.temporary_file_upload.rules', 'file|max:50');
    ...
    public function temporary_files_older_than_24_hours_are_cleaned_up_on_every_new_upload()
    {
        ...
        foreach (FileUploadConfiguration::storage()->allFiles() as $fileShortPath) {
            touch(FileUploadConfiguration::storage()->path($fileShortPath), ws_now()->subDays(2)->timestamp);
        }
    ...
    {
        ws_config()->set('livewire.temporary_file_upload.middleware', 'Tests\Unit\DummyMiddleware');
    ...
    {
        if (version_compare(ws_app()->version(), '9.2.0', '<')) {
    ...
    {
        ws_config()->set('livewire.temporary_file_upload.preview_mimes', ['pdf']);
...
class NonFileUploadComponent extends Component
{
    ...
    public function render() { return ws_app('view')->make('null-view'); }
}
