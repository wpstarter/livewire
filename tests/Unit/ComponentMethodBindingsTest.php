...
class ComponentWithBindings extends Component
{
    ...
    {
        $this->name = ws_collect([$model->value, $param])->join(':');
    }
    ...
    {
        $this->name = ws_collect([$foo, $model->value, $param])->join(':');
    }

    public function actionWithModelAndDependency(UrlGenerator $generator, ModelToBeBound $model, $param = 'param-default')
    {
        $this->name = ws_collect([$model->value, $generator->to('/some-url/'.$param)])->join(':');
    }
...
class ComponentWithMountInjections extends Component
{
    ...
    {
        $this->name = ws_collect([$generator->to('/some-url'), $model->value, $param])->join(':');
    }
