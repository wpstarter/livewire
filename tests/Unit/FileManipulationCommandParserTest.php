...
class FileManipulationCommandParserTest extends TestCase
{
    ...
    public function something($input, $component, $namespace, $classPath, $viewName, $viewPath)
    {
        $parser = new ComponentParser(
            'App\Http\Livewire',
            ws_resource_path('views/livewire'),
            $input
        );

        $this->assertEquals($component, $parser->component());
        $this->assertEquals($namespace, $parser->classNamespace());
        $this->assertEquals($this->normalizeDirectories(ws_app_path($classPath)), $this->normalizeDirectories($parser->classPath()));
        $this->assertEquals($viewName, $parser->viewName());
        $this->assertEquals($this->normalizeDirectories(ws_resource_path('views/'.$viewPath)), $this->normalizeDirectories($parser->viewPath()));
    }
