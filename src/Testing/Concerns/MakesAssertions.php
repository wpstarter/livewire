...
trait MakesAssertions
{
    ...
    public function assertPayloadSet($name, $value)
    {
        if (is_callable($value)) {
            PHPUnit::assertTrue($value(ws_data_get($this->payload['serverMemo']['data'], $name)));
        } else {
            PHPUnit::assertEquals($value, ws_data_get($this->payload['serverMemo']['data'], $name));
        }
    ...
    {
        if (is_callable($value)) {
            PHPUnit::assertFalse($value(ws_data_get($this->payload['serverMemo']['data'], $name)));
        } else {
            PHPUnit::assertNotEquals($value, ws_data_get($this->payload['serverMemo']['data'], $name));
        }
    ...
    protected function testEmitted($value, $params)
    {
        ...
        if (empty($params)) {
            $test = ws_collect(ws_data_get($this->payload, 'effects.emits'))->contains('event', '=', $value);
        } elseif (! is_string($params[0]) && is_callable($params[0])) {
            $event = ws_collect(ws_data_get($this->payload, 'effects.emits'))->first(function ($item) use ($value) {
                return $item['event'] === $value;
            });

            $test = $event && $params[0]($event['event'], $event['params']);
        } else {
            $test = (bool) ws_collect(ws_data_get($this->payload, 'effects.emits'))->first(function ($item) use ($value, $params) {
                return $item['event'] === $value
                    && $item['params'] === $params;
            });
    ...
    protected function testEmittedTo($target, $value)
    {
        ...
        return (bool) ws_collect(ws_data_get($this->payload, 'effects.emits'))->first(function ($item) use ($target, $value) {
            return $item['event'] === $value
                && $item['to'] === $target;
        });
    ...
    {
        return (bool) ws_collect(ws_data_get($this->payload, 'effects.emits'))->first(function ($item) use ($value) {
            return $item['event'] === $value
    ...
    public function assertDispatchedBrowserEvent($name, $data = null)
    {
        ...
        if (is_null($data)) {
            $test = ws_collect(ws_data_get($this->payload, 'effects.dispatches'))->contains('event', '=', $name);
        } elseif (is_callable($data)) {
            $event = ws_collect(ws_data_get($this->payload, 'effects.dispatches'))->first(function ($item) use ($name) {
                return $item['event'] === $name;
            });

            $test = $event && $data($event['event'], $event['data']);
        } else {
            $test = (bool) ws_collect(ws_data_get($this->payload, 'effects.dispatches'))->first(function ($item) use ($name, $data) {
                return $item['event'] === $name
                    && $item['data'] === $data;
            });
    ...
    public function assertNotDispatchedBrowserEvent($name)
    {
        if (! array_key_exists('dispatches', $this->payload['effects'])){
            ...
            $test = ws_collect($this->payload['effects']['dispatches'])->contains('event', '=', $name);
        }
    ...
    public function assertHasErrors($keys = [])
    {
        ...
        foreach ($keys as $key => $value) {
            if (is_int($key)) {
                ...
                $failed = ws_optional($this->lastValidator)->failed() ?: [];
    ...
    public function assertRedirect($uri = null)
    {
        ...
        if (! is_null($uri)) {
            PHPUnit::assertSame(ws_url($uri), ws_url($this->payload['effects']['redirect']));
        }
    ...
    {
        $downloadEffect = ws_data_get($this->lastResponse, 'original.effects.download');

        if ($filename) {
            PHPUnit::assertEquals(
                $filename,
                ws_data_get($downloadEffect, 'name')
            );
        } else {
        ...
        if ($content) {
            $downloadedContent = ws_data_get($this->lastResponse, 'original.effects.download.content');
        ...
        if ($contentType) {
            PHPUnit::assertEquals(
                $contentType,
                ws_data_get($this->lastResponse, 'original.effects.download.contentType')
            );
        }
