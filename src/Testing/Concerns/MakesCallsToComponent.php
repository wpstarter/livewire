...
trait MakesCallsToComponent
{
    ...
    public function syncUploadedFiles($name, $files, $isMultiple = false)
    {
        ...
        $this->sendMessage('callMethod', [
            'method' => 'startUpload',
            'params' => [$name, ws_collect($files)->map(function ($file) {
                return [
                    'name' => $file->name,
                    'size' => $file->getSize(),
        ...
        $newFileHashes = ws_collect($files)->zip($fileHashes)->mapSpread(function ($file, $fileHash) {
            return (string) str($fileHash)->replaceFirst('.', "-size={$file->getSize()}.");
        })->toArray();

        ws_collect($fileHashes)->zip($newFileHashes)->mapSpread(function ($fileHash, $newFileHash) use ($storage) {
