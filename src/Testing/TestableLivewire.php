...
/** @mixin \WpStarter\Testing\TestResponse */
class TestableLivewire
{
    ...
    public function __construct($name, $params = [], $queryParams = [])
    {
        ...
        if (class_exists($name)) {
            ...
            $componentClass = $name;
            ws_app('livewire')->component($name = str()->random(20), $componentClass);
        }
    ...
    public function updateComponent($output, $isInitial = false)
    {
        ...
        foreach ($output['serverMemo'] as $key => $newValue) {
            if ($key === 'data') {
                if ($isInitial) ws_data_set($this->payload, 'serverMemo.data', []);

                foreach ($newValue as $dataKey => $dataValue) {
                    ws_data_set($this->payload, 'serverMemo.data.'.$dataKey, $dataValue);
                }
    ...
    public function pretendWereMountingAComponentOnAPage($name, $params, $queryParams)
    {
        ...
        $wpStarterTestingWrapper = new MakesHttpRequestsWrapper(ws_app());

        $response = null;

        $wpStarterTestingWrapper->temporarilyDisableExceptionHandlingAndMiddleware(function ($wrapper) use ($randomRoutePath, &$response, $queryParams) {
    ...
    private function registerRouteBeforeExistingRoutes($path, $closure)
    {
        ...
        $existingRoutes = ws_app('router')->getRoutes();
        ...
        ws_app('router')->setRoutes($runningCollection);
    }
    ...
    public function callEndpoint($method, $url, $payload)
    {
        ...
        $wpStarterTestingWrapper->temporarilyDisableExceptionHandlingAndMiddleware(function ($wrapper) use (&$response, $method, $url, $payload) {
    ...
    public function get($property)
    {
        return ws_data_get(
            ...
            function () use ($property) {
                ...
                return ws_data_get($value, $nested);
            }
        );
    }
