...
class LifecycleManager
{
    ...
    {
        return ws_tap(new static, function ($instance) use ($payload) {
            $instance->request = new Request($payload);
            $instance->instance = ws_app('livewire')->getInstance($instance->request->name(), $instance->request->id());
        });
    }

    public static function fromInitialRequest($name, $id)
    {
        return ws_tap(new static, function ($instance) use ($name, $id) {
            $instance->instance = ws_app('livewire')->getInstance($name, $id);
            $instance->request = new Request([
                'fingerprint' => [
                    'id' => $id,
                    'name' => $name,
                    'locale' => ws_app()->getLocale(),
    ...
    {
        $name = ws_app('livewire')->getAlias(get_class($component), $component->getName());

        return ws_tap(new static, function ($instance) use ($component,  $name) {
    ...
    public function mount($params = [])
    {
        ...
        ws_collect(array_intersect_key($params, $this->instance->getPublicPropertiesDefinedBySubClass()))
        ...
        if (method_exists($this->instance, 'mount')) {
            try {
                ImplicitlyBoundMethod::call(ws_app(), [$this->instance, 'mount'], $params);
