...
abstract class Component
{
    ...
    public function __invoke(Container $container, Route $route)
    {
        ...
        try {
            ...
            if (method_exists($route,'getMissing') && $route->getMissing()) {
                return $route->getMissing()(ws_request());
            }
        ...
        if ($instance->redirectTo) {
            return ws_redirect()->response($instance->redirectTo);
        }
        ...
        return ws_app('view')->file(__DIR__."/Macros/livewire-view-{$layout['type']}.blade.php", [
            'view' => $layout['view'],
    ...
    public function bootIfNotBooted()
    {
        if (method_exists($this, $method = 'boot')) {
            ImplicitlyBoundMethod::call(ws_app(), [$this, $method]);
        }
    }

    public function initializeTraits()
    {
        foreach (ws_class_uses_recursive($class = static::class) as $trait) {
    ...
    {
        $namespace = ws_collect(explode('.', str_replace(['/', '\\'], '.', ws_config('livewire.class_namespace'))))
            ->map([Str::class, 'kebab'])
            ->implode('.');

        $fullName = ws_collect(explode('.', str_replace(['/', '\\'], '.', static::class)))
            ->map([Str::class, 'kebab'])
            ->implode('.');
    ...
    public function getQueryString()
    {
        ...
        return ws_collect(ws_class_uses_recursive($class = static::class))
            ->map(function ($trait) use ($class) {
                $member = 'queryString' . ws_class_basename($trait);
    ...
    public function renderToView()
    {
        ...
        $view = method_exists($this, 'render')
            ? ws_app()->call([$this, 'render'])
            : ws_view("livewire.{$this::getName()}");

        if (is_string($view)) {
            $view = ws_app('view')->make(CreateBladeView::fromString($view));
        }

        ws_throw_unless($view instanceof View,
    ...
    protected function ensureViewHasValidLivewireLayout(View $view)
    {
        ...
        if (!$isValid) {
            $view->layout($layout['view'] ?? ws_config('livewire.layout'), $layout['params'] ?? []);
    ...
    public function output($errors = null)
    {
        ...
        // of this method.
        $engine = ws_app('view.engine.resolver')->resolve('blade');
        ...
        $previouslySharedErrors = ws_app('view')->getShared()['errors'] ?? new ViewErrorBag;
        $previouslySharedInstance = ws_app('view')->getShared()['_instance'] ?? null;
        ...
        ws_app('view')->share('errors', $errors);
        ws_app('view')->share('_instance', $this);

        $output = $view->render();

        ws_app('view')->share('errors', $previouslySharedErrors);
        ws_app('view')->share('_instance', $previouslySharedInstance);
    ...
    public function forgetComputed($key = null)
    {
        ...
        ws_collect($keys)->each(function ($i) {
    ...
    public function __get($property)
    {
        ...
        if (method_exists($this, $computedMethodName = 'get'.$studlyProperty.'Property')) {
            ...
            return $this->computedPropertyCache[$property] = ws_app()->call([$this, $computedMethodName]);
        }
