...
class TemporaryUploadedFile extends UploadedFile
{
    ...
    {
        if (ws_app()->runningUnitTests() && str($this->getfilename())->contains('-size=')) {
    ...
    {
        if ((FileUploadConfiguration::isUsingS3() or FileUploadConfiguration::isUsingGCS()) && ! ws_app()->runningUnitTests()) {
        ...
        if (method_exists($this->storage->getAdapter(), 'getTemporaryUrl') || ! $this->isPreviewable()) {
            ...
            return $this->storage->temporaryUrl($this->path, ws_now()->addDay());
        }

        return URL::temporarySignedRoute(
            'livewire.preview-file', ws_now()->addMinutes(30), ['filename' => $this->getFilename()]
        );
    }

    public function isPreviewable()
    {
        $supportedPreviewTypes = ws_config('livewire.temporary_file_upload.preview_mimes', [
    ...
    {
        return base64_decode(ws_head(explode('-', ws_last(explode('-meta', str($path)->replace('_', '/'))))));
    }
    ...
    public static function canUnserialize($subject)
    {
        ...
        if (is_array($subject)) {
            return ws_collect($subject)->contains(function ($value) {
    ...
    public static function unserializeFromLivewireRequest($subject)
    {
        if (is_string($subject)) {
            ...
            if (str($subject)->startsWith('livewire-files:')) {
                ...
                return ws_collect($paths)->map(function ($path) { return static::createFromLivewire($path); })->toArray();
            }
        }
    ...
    {
        return 'livewire-files:'.json_encode(ws_collect($files)->map->getFilename());
    }
}
