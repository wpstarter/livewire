...
class FileManipulationCommand extends Command
{
    ...
    {
        $namespace = str(ws_config('livewire.class_namespace'))->replaceFirst(ws_app()->getNamespace(), '');

        $livewireFolder = ws_app_path($namespace->explode('\\')->implode(DIRECTORY_SEPARATOR));
