...
class SupportBrowserHistory
{
    ...
    function __construct()
    {
        ...
        Livewire::listen('component.dehydrate.initial', function (Component $component, Response $response) {
            if (($referer = ws_request()->header('Referer')) && ws_request()->header('x-livewire')) {
                ...
                $response->effects['path'] = ws_url()->current().$this->stringifyQueryParams($queryParams);
            }
        });

        Livewire::listen('component.dehydrate.subsequent', function (Component $component, Response $response) {
            if (! $referer = ws_request()->header('Referer')) return;
    ...
    protected function shouldSendPath($component, $route = null)
    {
        ...
        $route = $route ?? ws_app('router')->current();
    ...
    {
        if (empty($referer = ws_request()->header('Referer'))) return [];
    ...
    protected function buildPathFromRoute($component, $route, $queryString)
    {
        ...
        return ws_app(UrlGenerator::class)->toRoute($route, $boundParameters + $queryString->toArray(), true);
    }
    ...
    protected function mergeComponentPropertiesWithExistingQueryParamsFromOtherComponentsAndTheRequest($component)
    {
        if (! $this->mergedQueryParamsFromDehydratedComponents) {
            $this->mergedQueryParamsFromDehydratedComponents = ws_collect($this->getExistingQueryParams());
        }

        $excepts = $this->getExceptsFromComponent($component);

        $this->mergedQueryParamsFromDehydratedComponents = ws_collect(ws_request()->query())
    ...
    {
        return ws_collect($component->getQueryString())
