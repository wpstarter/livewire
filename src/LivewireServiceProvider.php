...
use WpStarter\View\ComponentAttributeBag;
...
class LivewireServiceProvider extends ServiceProvider
{
    ...
    protected function registerComponentAutoDiscovery()
    {
        ...
        $this->app->singleton(LivewireComponentsFinder::class, function () use ($defaultManifestPath) {
            return new LivewireComponentsFinder(
                new Filesystem,
                ws_config('livewire.manifest_path') ?: $defaultManifestPath,
                ComponentParser::generatePathFromNamespace(
                    ws_config('livewire.class_namespace')
                )
            );
        });
    }
    ...
    protected function registerRoutes()
    {
        RouteFacade::post('/livewire/message/{name}', HttpConnectionHandler::class)
            ...
            ->middleware(ws_config('livewire.middleware_group', ''));
    ...
    protected function registerViewMacros()
    {
        ...
        if (method_exists(ComponentAttributeBag::class, 'macro')) {
            ComponentAttributeBag::macro('wire', function ($name) {
                $entries = ws_head((array) $this->whereStartsWith('wire:'.$name));

                $directive = ws_head(array_keys($entries));
                $value = ws_head(array_values($entries));
    ...
    protected function registerTagCompiler()
    {
        if (method_exists($this->app['blade.compiler'], 'precompiler')) {
            $this->app['blade.compiler']->precompiler(function ($string) {
                return ws_app(LivewireTagCompiler::class)->compile($string);
            });
        }
    }
    ...
    protected function registerPublishables()
    {
        $this->publishesToGroups([
            __DIR__.'/../dist' => ws_public_path('vendor/livewire'),
        ], ['livewire', 'livewire:assets']);

        $this->publishesToGroups([
            __DIR__.'/../config/livewire.php' => ws_base_path('config/livewire.php'),
