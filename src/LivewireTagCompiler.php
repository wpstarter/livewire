...
class LivewireTagCompiler extends ComponentTagCompiler
{
    ...
    protected function compileLivewireSelfClosingTags($value)
    {
        ...
        return preg_replace_callback($pattern, function (array $matches) {
            ...
            $attributes = ws_collect($attributes)->mapWithKeys(function ($value, $key) {
            ...
            if ($component === 'dynamic-component' || $component === 'is') {
                if(! isset($attributes['component'])) {
                    $dynamicComponentExists = ws_rescue(function() use ($component, $attributes) {
                        // Need to run this in rescue otherwise running this during a test causes Livewire directory not found exception
                        return $component === 'dynamic-component' && ws_app('livewire')->getClass('dynamic-component');
                    });
    ...
    {
        return ws_collect($attributes)
                ->map(function (string $value, string $attribute) use ($escapeBound) {
                    return $escapeBound && isset($this->boundAttributes[$attribute]) && $value !== 'true' && ! is_numeric($value)
                                ? "'{$attribute}' => \WpStarter\View\Compilers\BladeCompiler::sanitizeComponentAttribute({$value})"
                                : "'{$attribute}' => {$value}";
                })
                ->implode(',');
    }
