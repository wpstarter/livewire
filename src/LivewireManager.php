...
class LivewireManager
{
    ...
    protected $persistentMiddleware = [
        ...
        \WpStarter\Auth\Middleware\AuthenticateWithBasicAuth::class,
    ...
    {
        $finder = ws_app(LivewireComponentsFinder::class);
        ...
        ws_throw_unless($class, new ComponentNotFoundException(
    ...
    public function getInstance($component, $id)
    {
        ...
        ws_throw_unless(class_exists($componentClass), new ComponentNotFoundException(
    ...
    public function actingAs(Authenticatable $user, $driver = null)
    {
        ...
        ws_auth()->guard($driver)->setUser($user);

        ws_auth()->shouldUse($driver);
...
HTML;
    ...
    protected function javaScriptAssets($options)
    {
        ...
        $assetsUrl = ws_config('livewire.asset_url') ?: rtrim($options['asset_url'] ?? '', '/');

        $appUrl = ws_config('livewire.app_url')
            ?: rtrim($options['app_url'] ?? '', '/')
            ?: $assetsUrl;

        $jsLivewireToken = ws_app()->has('session.store') ? "'" . ws_csrf_token() . "'" : 'null';
        ...
        if (file_exists(ws_public_path('vendor/livewire/manifest.json'))) {
            $publishedManifest = json_decode(file_get_contents(ws_public_path('vendor/livewire/manifest.json')), true);
            $versionedFileName = $publishedManifest['/livewire.js'];

            $fullAssetPath = ($this->isRunningServerless() ? ws_config('app.asset_url') : $assetsUrl).'/vendor/livewire'.$versionedFileName;
...
HTML;
    ...
    public function isProbablyLivewireRequest()
    {
        ...
        return ws_request()->hasHeader('X-Livewire');
    }

    public function originalUrl()
    {
        if ($this->isDefinitelyLivewireRequest()) {
            return ws_url()->to($this->originalPath());
        }

        return ws_url()->current();
    }
    ...
    public function originalPath()
    {
        if ($this->isDefinitelyLivewireRequest()) {
            ...
            // that still used "url".
            if (isset(ws_request('fingerprint')['url'])) {
                return str(ws_request('fingerprint')['url'])->after(ws_request()->root());
            }

            return ws_request('fingerprint.path');
        }

        return ws_request()->path();
    }
    ...
    public function originalMethod()
    {
        if ($this->isDefinitelyLivewireRequest()) {
            ...
            return ws_request('fingerprint.method', 'POST');
        }

        return ws_request()->method();
    }
    ...
    public function setBackButtonCache()
    {
         ...
         */
        $this->shouldDisableBackButtonCache = ! ws_config('livewire.back_button_cache', false);
    }
