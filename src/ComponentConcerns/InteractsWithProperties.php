...
trait InteractsWithProperties
{
    public function handleHydrateProperty($property, $value)
    {
        ...
        foreach (array_diff(ws_class_uses_recursive($this), class_uses(self::class)) as $trait) {
            $method = 'hydratePropertyFrom'.ws_class_basename($trait);
    ...
    public function handleDehydrateProperty($property, $value)
    {
        ...
        foreach (array_diff(ws_class_uses_recursive($this), class_uses(self::class)) as $trait) {
            $method = 'dehydratePropertyFrom'.ws_class_basename($trait);
    ...
    public function getPropertyValue($name)
    {
        ...
        if ($this->containsDots($name)) {
            return ws_data_get($value, $this->afterFirstDot($name));
        }
    ...
    {
        return ws_head(explode('.', $subject));
    }
    ...
    {
        return ws_collect((new \ReflectionObject($this))->getProperties(\ReflectionMethod::IS_PUBLIC))
