...
trait ValidatesInput
{
    ...
    public function errorBagExcept($field)
    {
        ...
        return new MessageBag(
            ws_collect($this->getErrorBag())
                ->reject(function ($messages, $messageKey) use ($fields) {
                    return ws_collect($fields)->some(function ($field) use ($messageKey) {
    ...
    {
        if (empty($this->getRules())) return ws_collect();

        return ws_collect($this->getRules())
    ...
    public function hasRuleFor($dotNotatedProperty)
    {
        ...
        if ($dotNotatedProperty !== $propertyWithStarsInsteadOfNumbers) {
            return ws_collect($this->getRules())->keys()->contains($propertyWithStarsInsteadOfNumbers);
        }
    ...
    public function validateOnly($field, $rules = null, $messages = [], $attributes = [])
    {
        ...
        $rulesForField = ws_collect($rules)
    ...
    protected function providedOrGlobalRulesMessagesAndAttributes($rules, $messages, $attributes)
    {
        ...
        ws_throw_if(empty($rules), new MissingRulesException($this::getName()));
    ...
    {
        return ws_collect($data)->map(function ($value) {
