...
trait HandlesActions
{
    public function syncInput($name, $value, $rehash = true)
    {
        ...
        $this->callBeforeAndAfterSyncHooks($name, $value, function ($name, $value) use ($propertyName, $rehash) {
            ...
            if ($this->containsDots($name)) {
                ...
                $results = [];
                $results[$targetKey] = ws_data_get($this->{$propertyName}, $targetKey, []);

                // Merge in new data.
                ws_data_set($results, $keyName, $value);

                // Re-assign data to model.
                ws_data_set($this->{$propertyName}, $targetKey, $results[$targetKey]);
            } else {
                $this->{$name} = $value;
            }
    ...
    public function callMethod($method, $params = [], $captureReturnValueCallback = null)
    {
        ...
        switch ($method) {
            case '$sync':
                ...
                $this->syncInput($prop, ws_head($params));

                return;

            case '$set':
                $prop = array_shift($params);
                $this->syncInput($prop, ws_head($params), $rehash = false);
            ...
            case '$toggle':
                ...
                if ($this->containsDots($prop)) {
                    ...
                    $currentValue = ws_data_get($this->{$propertyName}, $targetKey);
                } else {
        ...
        if (! method_exists($this, $method)) {
            ws_throw_if($method === 'startUpload', new MissingFileUploadsTraitException($this));

            throw new MethodNotFoundException($method, $this::getName());
        }

        ws_throw_unless($this->methodIsPublicAndNotDefinedOnBaseClass($method), new NonPublicComponentMethodCall($method));

        $returned = ImplicitlyBoundMethod::call(ws_app(), [$this, $method], $params);
    ...
    {
        return ws_collect((new \ReflectionClass($this))->getMethods(\ReflectionMethod::IS_PUBLIC))
